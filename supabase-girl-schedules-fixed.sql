...
CREATE TABLE girl_schedules (
  ...
  girl_id UUID NOT NULL,  -- ← TEXT から UUID に変更
  shop_id UUID NOT NULL,  -- ← TEXT から UUID に変更
...
-- インデックス作成（検索高速化）
CREATE INDEX idx_girl_schedules_girl_id ON girl_schedules(girl_id);
CREATE INDEX idx_girl_schedules_shop_id ON girl_schedules(shop_id);
CREATE INDEX idx_girl_schedules_date ON girl_schedules(date);
CREATE INDEX idx_girl_schedules_status ON girl_schedules(status);
CREATE INDEX idx_girl_schedules_instant ON girl_schedules(instant_available);
...
-- 完了メッセージ
SELECT 'girl_schedules table created successfully! UUID types fixed!' as result;
